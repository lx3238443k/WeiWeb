...
;(function(_){
  ...
  function Slider( opt ){
    ...
    // 容器节点 以及 样式设置
    this.container = this.container || document.getElementById('g-main');
  ...
  _.extend( Slider.prototype, {
    ...
    _onNav: function(pageIndex, slideIndex){
      ...
      for(var i =-1; i<= 1; i++){
        ...
        var img = slides[index].querySelector('image')
        if(!img){
          img = document.createElement('img');
          slides[index].appendChild(img);
        }
        img.src = './image/banner0' + ( this._normIndex(pageIndex + i, this.pageNum)+1 ) + '.jpg';
      }
